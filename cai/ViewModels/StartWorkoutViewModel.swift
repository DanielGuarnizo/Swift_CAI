...
class Webservice_StartWorkoutViewModel {
    func get_templates_workouts(completion: @escaping(Result<[TemplateWorkout], APIError>)-> Void){
        // 1.Create the URL
        guard let url = URL(string: "http://localhost/api/v1/templates/workouts") else {
        ...
        //print("START WORKOUT VIEW token:")
        //print(token ?? "")
...
class StartWorkoutViewModel: ObservableObject {
    ...
    func loadWorkouts() {
        ...
        let webservice = Webservice_StartWorkoutViewModel()
